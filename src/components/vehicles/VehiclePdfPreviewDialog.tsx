...
interface VehiclePdfPreviewDialogProps {
  isOpen: boolean;
  onClose: () => void;
  vehicles: Vehicle[];
...
export const VehiclePdfPreviewDialog: React.FC<VehiclePdfPreviewDialogProps> = ({
  isOpen,
  onClose,
  vehicles,
  filterType,
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      ...
      toast.success("PDF gerado com sucesso!", {
        description: "O relatório de veículos foi baixado para seu computador.",
      });
      onClose();
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl w-full h-auto flex flex-col p-0 bg-card/20 backdrop-blur-md border border-border/50">
        <DialogHeader className="p-6 pb-4 border-b">
          <DialogTitle>Relatório de Veículos</DialogTitle>
          <DialogDescription>
